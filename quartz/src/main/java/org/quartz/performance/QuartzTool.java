...
public class QuartzTool implements Tool {
    ...
    private void installSchemaAndConfigureDataSource(Properties props, AbstractSqlDataStore dataStore) throws Exception {
        HikariDataSource dataSource = dataStore.dataSource();
        ...
        props.setProperty("org.quartz.dataSource.myDS.driver", dataSource.getDriverClassName());
        props.setProperty("org.quartz.dataSource.myDS.URL", dataSource.getJdbcUrl());
        props.setProperty("org.quartz.dataSource.myDS.user", dataSource.getUsername());
        props.setProperty("org.quartz.dataSource.myDS.password", dataSource.getPassword());
        ...
        switch (dataStore) {
            case DB2DataStore db2DataStore:
                ...
                QuartzDatabaseCreator.install(dataStore.dataSource(), "org/quartz/impl/jdbcjobstore/tables_db2_v95.sql");
                break;
            case MariaDBDataStore mariaDBDataStore:
                props.setProperty("org.quartz.jobStore.driverDelegateClass", "org.quartz.impl.jdbcjobstore.StdJDBCDelegate");
                QuartzDatabaseCreator.install(dataStore.dataSource(), "org/quartz/impl/jdbcjobstore/tables_mysql_innodb.sql");
                break;
            ...
            case OracleDataStore oracleDataStore:
                ...
                QuartzDatabaseCreator.install(dataStore.dataSource(), "org/quartz/impl/jdbcjobstore/tables_oracle23.sql");
                break;
            case PostgresDataStore postgres:
                ...
                QuartzDatabaseCreator.install(dataStore.dataSource(), "org/quartz/impl/jdbcjobstore/tables_postgres.sql");
                break;
