...
public class ScenarioReleem extends AbstractJobRunrProScenario {
    ...
    public ScenarioReleem(DataStore dataStore, String[] args) {
    ...
    protected Instant waitForJobsToComplete(Instant startTime) {
        while (true) {
            Instant previousRunEndTime = super.waitForJobsToComplete(startTime);

            LOGGER.info("Jobs were all processed - start creating more jobs");
            int totalAmountOfJobs = parseInt(getArg("amount", "0").replace("_", ""));
            int amountOfJobsCreated = 0;
            amountOfJobsCreated += createJobsInDynamicQueues((int) (totalAmountOfJobs * 0.5));
            amountOfJobsCreated += createBatchJobs((int) (totalAmountOfJobs * 0.3));
            amountOfJobsCreated += createAwaitingJobs((int) (totalAmountOfJobs * 0.1));
            amountOfJobsCreated += createScheduledJobs((int) (totalAmountOfJobs * 0.1));
            Instant jobCreationEndTime = Instant.now();

            LOGGER.info("Created more jobs");
            scenarioResult.setAmountOfJobsCreated(scenarioResult.getCreatedJobs() + amountOfJobsCreated, Duration.between(previousRunEndTime, jobCreationEndTime));
        }
    }
