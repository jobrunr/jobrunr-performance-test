...
public class LogBook {
    ...
    public static void append(BackgroundJobServer backgroundJobServer, ScenarioResult scenarioResult) {
        ...
        try (CsvWriter csv = CsvWriter.builder().build(logBookPath, StandardOpenOption.CREATE, StandardOpenOption.APPEND)) {
            if (addHeader)
                csv.writeRecord("Date & Time", "Host name", "Java version", "JobRunr type", "JobRunr version", "StorageProvider", "amount of jobs", "creation duration", "processing duration", "job throughput (jobs / sec)");
            csv.writeRecord(Instant.now().toString(), InetAddress.getLocalHost().getHostName(), getJavaVersion(), getJobRunrType(backgroundJobServer), getJobRunrVersion(backgroundJobServer), backgroundJobServer.getStorageProvider().getStorageProviderInfo().getName(), String.valueOf(scenarioResult.getAmountOfJobs()), scenarioResult.getCreationDuration().toString(), scenarioResult.getProcessingDuration().toString(), String.format(Locale.US, "%.2f", (double) scenarioResult.getAmountOfJobs() / (scenarioResult.getProcessingDuration().toSeconds())));
        } catch (IOException e) {
